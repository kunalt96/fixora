...
export default function Errors() {
  ...
  const handleGenerateApiKey = async () => {
    try {
      const token = await getToken();
      const res = await fetch(`${import.meta.env.VITE_API_URL}/generate-apiKey`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
    ...
    <div className="min-h-screen bg-gray-900 text-white">
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-4">
            ...
            <span className="text-gray-400 text-sm">AI Error Intelligence</span>
          </div>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={() => logout()}
              className="bg-gray-700 hover:bg-gray-600 text-white px-4 py-2 rounded font-medium transition"
            >
              Logout
            </button>
          </div>
        ...
        {/* ERROR LIST */}
        {filteredErrors.length === 0 && <p className="text-gray-400">No errors found</p>}
          ...
          {filteredErrors.map((err) => (
            ...
            >
              <div className="flex justify-between items-center">
                <h2 className="text-lg font-semibold text-red-400">{err.message}</h2>
                <span className="text-xs bg-red-500 px-2 py-1 rounded">Error</span>
              </div>
              <div className="mt-2 text-sm text-gray-300">
                <p className="text-red-400 font-bold">{err.count} occurrences</p>
                <p className="truncate">URL: {err.url}</p>
                <p>Last Seen: {new Date(err.lastSeen).toLocaleString()}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {selectedError && (
        <div className="fixed right-0 top-0 h-full w-[400px] bg-gray-900 border-l border-gray-700 p-5 overflow-y-auto">
          <h2 className="text-xl font-bold mb-4 text-blue-400">Error Details</h2>
          <p className="mb-2">
          ...
          <p className="mb-4 text-sm text-gray-400">
            Last Seen: {new Date(selectedError.lastSeen).toLocaleString()}
          </p>
          <button
      ...
      {analyses && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center">
          <div className="bg-gray-900 text-white p-6 rounded-xl w-[500px] border border-gray-700">
            <h2 className="text-xl font-bold mb-4 text-green-400">AI Debug Analysis</h2>
            {loading ? (
            ...
            <button onClick={() => setAnalyses(null)} className="mt-4 px-4 py-2 bg-red-600 rounded">
              Close
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-gray-900 border border-blue-500/50 shadow-[0_0_20px_rgba(59,130,246,0.2)] p-8 rounded-2xl w-[450px] text-center">
            ...
            <p className="text-sm text-gray-400 mb-6">
              Please copy this key and keep it safe. It is required to authenticate your Fixora SDK
            ...
            <div className="bg-gray-800 border border-gray-700 rounded-lg p-4 mb-6 flex items-center justify-between">
              <code className="text-[#4ade80] font-mono tracking-wider break-all">
                {generatedApiKey}
              </code>
              <button
